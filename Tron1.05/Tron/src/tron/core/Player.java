...
public class Player {
    ...
    public boolean computeMove() {
        ...
        try {
            ...
            endTime /= 1000000; // nano -> ms
            if(endTime > stepTime) {
                System.out.println(getName() + " took " + endTime +
                        "ms exceeding maximum step time of " + stepTime + "ms.");
                JOptionPane.showMessageDialog(null, getName() + " took " + endTime +
                        "ms exceeding maximum step time of " + stepTime + "ms.");
                return false;
            }
        } catch(Exception e) {
    ...
    public void step() {
        switch (direction) {
            case Game.NORTH: y++;    break;
            case Game.SOUTH: y--;    break;
            case Game.EAST:  x++;    break;
            case Game.WEST:  x--;    break;
        }
    }
